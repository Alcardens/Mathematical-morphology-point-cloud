#include "point_cloud_dilation.h"
#include "shaders.h"
#include <stdexcept>
#include <iostream>
#include <algorithm>
...
GLuint PointCloudDilator::compile_compute(const char* src) {
...
PointCloudDilator::PointCloudDilator(const DilationConfig& cfg)
    : m_cfg(cfg),
      ...
      })
{
    ...
    m_prog_dilate = compile_compute(SRC_DILATE);
}

PointCloudDilator::~PointCloudDilator() {
...
void PointCloudDilator::set_structuring_element(
...
void PointCloudDilator::dispatch_dilation(uint32_t input_count) {
    uint32_t zero = 0;
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_ssbo_cand_cnt);
...
std::vector<std::array<float, 3>> PointCloudDilator::get_result() const {
    return m_hash.download_points();
}

Point_set PointCloudDilator::get_result_cgal() const {
    auto pts = get_result();
    Point_set ps;
    for (auto& p : pts)
