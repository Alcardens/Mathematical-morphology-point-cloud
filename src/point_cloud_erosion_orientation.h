...
struct OrientationErosionConfig {
    float    cell_size;
    float    min_dist;
    uint32_t table_size;
    uint32_t max_input;
...
class PointCloudOrientationEroder {
public:
    explicit PointCloudOrientationEroder(const OrientationErosionConfig& cfg);
    ~PointCloudOrientationEroder();
...
private:
    OrientationErosionConfig m_cfg;
